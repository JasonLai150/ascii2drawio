...
**Node detection** — `find_rectangles` / `_close_rect`: closed rectangles from `┌`/`+` corners, with ±1 column-drift tolerance on walls/corners and per-row wall tracking for label extraction. `_close_rect` first tries `_close_by_walls` (standard ±1 wall walk); if that fails it falls back to `_close_by_edges`, which recovers a box whose middle-row walls drifted ≥2 cols but whose top+bottom edges are fully formed and column-aligned (strong evidence — only fires on real malformed boxes), locating the drifted walls with a wider ±3 search for label extraction.
